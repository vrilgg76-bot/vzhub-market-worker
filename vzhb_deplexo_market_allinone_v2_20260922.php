 ...
 * - persistent M1/M5/M30/H1/H2/H4 OHLC generated by the worker itself
 *
...
function randomTick(float $currentPrice, float $maxTickPercent, float $minPrice, float $maxPrice): array {
    ...
    // No artificial upper price limit.
            ...
            if (!hash_equals($adminKey, (string)$providedKey)) {
                ...
                if ($action === 'stop') {
                    ...
                    if ($mode === 'percent') {
                        ...
                        $target = $direction === 'down' ? $minPrice : ($price + ($step * $duration));
                    } else {
                        $response = jsonResponse(400, ['ok' => false, 'error' => 'invalid_mode']);
                        goto control_done;
                    }

                    $target = max($minPrice, (float)$target);
                    if ($target == $price) {
